...
字段说明：`title` 必填；`category`、`tag`、`due`、`dueTime`、`priority`、`repeat`、`notes`、`reminderAt`、`subtasks`、`done` 可选。日期使用 `YYYY-MM-DD`，时间使用 `HH:mm`；`priority` 为 1-4，`repeat` 可使用 `daily`、`weekdays`、`weekly`、`monthly`、`yearly`。导入会替换当前任务列表，分类会自动合并，并提供一次“撤销导入”。
